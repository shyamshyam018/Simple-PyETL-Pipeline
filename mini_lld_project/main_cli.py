...
# Global variable to hold the DataFrame (In-Memory Processing)
# This simulates the data being passed between tasks in a workflow management system.
global_df = None 
...
def choose_source_file():
    ...
    while True:
        try:
            choice = int(input("Enter number of file to load: ")) - 1
...
def pipeline_step_extract_load():
    ...
    # L: Staging Load
    save_staging_data(df, stage_name=os.path.basename(file_path).split('.')[0])
    
    global_df = df # Data is now ready for transformation
    logging.info("Task 1 Completed successfully. Data is now staged.")
    return True
...
def pipeline_step_transform():
    ...
    df_current = global_df.copy() # Work on a copy
    ...
    while True:
        ...
        print("5. Discard changes and Exit Transformation")
        
        choice = input("Enter transformation choice (1-5): ")
        
        if choice == '1':
            df_current = clean_names(df_current)
        elif choice == '2':
            df_current = filter_data(df_current)
        elif choice == '3':
            df_current = aggregate_data(df_current)
        elif choice == '4':
            global_df = df_current # Commit changes to global state
...
def pipeline_step_final_load():
    ...
    logging.info("Task 3: Starting Final Load.")
    
    # L: Final Load
    save_final_data(global_df, final_name="FINAL_ANALYTICS")
    
    # Cleanup global state after successful load
    global_df = None
...
def run_full_pipeline_scheduled():
    ...
    print("\n" + "#"*70)
    logging.info("--- SCHEDULER TRIGGERED FULL PIPELINE RUN ---")
    
    if not pipeline_step_extract_load():
        logging.error("Full Pipeline Halted at E&L.")
        return
    
    # Run the transformation with default, non-interactive settings for scheduling
    # For a scheduled run, we apply a non-interactive default transform:
    global global_df
    if global_df is not None:
        df_scheduled = global_df.copy()
        df_scheduled = clean_names(df_scheduled) # Default clean
        # Default Filter: Remove rows where 'name' is missing
        df_scheduled = df_scheduled.dropna(subset=['name']) 
        global_df = df_scheduled
        logging.info("Scheduled Task: Applied default cleaning and filtering.")
...
def schedule_task():
    ...
    print("Full Pipeline is scheduled to run every 30 seconds.")
...
def main_cli():
    ...
    # Check dependencies
    try:
        if not all([load_source_data, clean_names, schedule.every]):
             raise ImportError("Missing components.")
    ...
    while True:
        ...
        print("5. View Current In-Memory Data (After E&L/T)")
        print("6. Exit")
